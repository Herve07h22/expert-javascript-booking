 ...
 *
 * Ne jamais afficher error.message brut : il est écrit pour vous.
 * Ne jamais afficher une erreur sans dire quoi faire.
 */
const messages: Record<string, (details: Details) => string> = {
  ...
  ACCOMMODATION_TOO_SMALL: ({ capacity, guests }) =>
    `Ce logement accueille ${String(capacity)} personnes, vous êtes ${String(guests)}.`,
...
export function toMessage(error: Partial<ApiErrorShape> | null): string {
  const template = error?.code ? messages[error.code] : undefined;
