...
import { errorCode, stay } from "../../tests/helpers.js";
import { occupancy } from "../../tests/helpers.js";
...
it("A tenant cannot book an accommodation in the past", async () => {
  ...
  expect(errorCode(session)).toBe(StayMustStartInTheFuture(today).code);
});
...
it("A booking needs at least one adult", async () => {
  ...
  expect(errorCode(session)).toBe(NeedsAtLeastOneAdult(0).code);
});
...
it("An anonymous visitor cannot book", async () => {
  ...
  expect(errorCode(session)).toBe("SHOULD_BE_LOGGED");
  const bookings =
    await app.dependencies.bookings.listBookingsForAccommodationId(
      "accommodation-1"
    );
...
it("A tenant cannot book an accommodation that does not exist", async () => {
  ...
  expect(errorCode(session)).toBe(UnknownAccommodation("accommodation-42").code);
});
...
it("A tenant cannot book an accommodation already booked", async () => {
  ...
  expect(errorCode(session)).toBe(AccommodationNotAvailable("accommodation-1").code);
