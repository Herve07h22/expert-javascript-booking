 ...
 */
export function routes(app, { logger, isProduction }) {
  const router = Router();

  const handle = (scenario) => async (request, response) => {
    try {
      ...
      response
        .status(200)
        .json({ data: context.data ?? context.session().currentUser ?? null });
    } catch (error) {
  ...
  router.post("/api/sessions", async (request, response) => {
    ...
    response.cookie("session", context.token, {
      ...
      secure: isProduction, // en développement sur http, Secure ne partirait pas
